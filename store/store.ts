  ...
  persist(
    (set: any, get: any) => ({
      ...
      syncLocalToSupabase: async () => {
        ...
        // Only locally-created or locally-updated tasks
        const localTasks = mainData.filter((t: any) => t.source === "local");
        ...
        try {
          ...
          const syncPayload = localTasks.map((task: any) => ({
            id: task.id, // REQUIRED
            user_id: userData.id, // IMPORTANT (RLS safety)
            board_id: task.board_id,
            completed: task.completed,
            position: task.position,
            title: task.title,
          ...
          const { data, error } = await supabase.rpc("sync_tasks", {
            payload: syncPayload,
          });
          ...
          // Replace local store with server state
          set({
            mainData: freshData,
            snackbar: {
              show: true,
